...
agent_models = {"A": AGENT_MODEL, "B": AGENT_MODEL, "C": AGENT_MODEL}

def agent_label(agent: str) -> str:
    return f"{agent} ({agent_models.get(agent, '?')})"

COALITION_VALUES = {"A+B": 100, "A+C": 90, "B+C": 30, "A+B+C": 120}

def is_in_core(coalition: str, allocations: dict) -> bool:
    """Check if allocation is in the core."""
    if not coalition or not allocations or coalition != "A+B+C":
        return False
    
    x_A = allocations.get("A", 0)
    x_B = allocations.get("B", 0)
    x_C = allocations.get("C", 0)
    
    if (x_A + x_B >= COALITION_VALUES["A+B"] and
        x_A + x_C >= COALITION_VALUES["A+C"] and
        x_B + x_C >= COALITION_VALUES["B+C"]):
        return True
    return False


def is_exact_equal_division(coalition: str, allocations: dict) -> bool:
    """Check for exact equal division."""
    if coalition != "A+B+C":
        return False
    ...
    x_B = allocations.get("B", 0)
    x_C = allocations.get("C", 0)
    equal_share = COALITION_VALUES["A+B+C"] / 3
    
    return x_A == equal_share and x_B == equal_share and x_C == equal_share


def is_in_edc(coalition: str, allocations: dict) -> bool:
    """Check if allocation is in Equal Division Core."""
    if not coalition or not allocations or coalition != "A+B+C":
        return False
    ...
    ab_equal = COALITION_VALUES["A+B"] / 2
    if ab_equal > x_A and ab_equal > x_B:
        return False
    
    ac_equal = COALITION_VALUES["A+C"] / 2
    if ac_equal > x_A and ac_equal > x_C:
        return False
    
    bc_equal = COALITION_VALUES["B+C"] / 2
    if bc_equal > x_B and bc_equal > x_C:
        return False
    
    abc_equal = COALITION_VALUES["A+B+C"] / 3
    if abc_equal > x_A and abc_equal > x_B and abc_equal > x_C:
        return False
...
(Note: The game cannot end until at least turn {MIN_TURNS})
...
def wide_end_session(*, final_coalition, allocations, formed, invalid, actual_turns, invalid_reason=""):
    ...
    if formed and final_coalition == "A+B+C" and allocations:
        in_core = is_in_core(final_coalition, allocations)
        in_edc = is_in_edc(final_coalition, allocations)
        exact_equal = is_exact_equal_division(final_coalition, allocations)
        
        CURRENT_WIDE_ROW["in_core"] = 1 if in_core else 0
        CURRENT_WIDE_ROW["in_edc"] = 1 if in_edc else 0
        CURRENT_WIDE_ROW["exact_equal_division"] = 1 if exact_equal else 0
        CURRENT_WIDE_ROW["other"] = 1 if (not in_core and not in_edc and not exact_equal) else 0
    else:
        CURRENT_WIDE_ROW["in_core"] = 0
        CURRENT_WIDE_ROW["in_edc"] = 0
        CURRENT_WIDE_ROW["exact_equal_division"] = 0
        CURRENT_WIDE_ROW["other"] = 0
...
def run_single_treatment_runs(treatment="public", use_csv: bool = True, num_runs=15):
    run_timestamp = datetime.now()
    model_assignment = {"A": AGENT_MODEL, "B": AGENT_MODEL, "C": AGENT_MODEL}
    
    print("\n" + "="*80)
    print(f"FACILITATOR-CONTROLLED NEGOTIATION EXPERIMENT")
    print("="*80)
    print(f"Facilitator Model: {FACILITATOR_MODEL}")
    print(f"Agent Model (all agents): {AGENT_MODEL}") 
    ...
    for run_id in range(1, num_runs + 1):
        first_speaker = FIRST_SPEAKERS[(run_id - 1) % len(FIRST_SPEAKERS)]
    ...
    print(f"\nSUMMARY: {len(successful_coalitions)}/{num_runs} successful coalitions.")
